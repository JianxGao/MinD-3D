...
class MinD3D(nn.Module):
    def __init__(self, config, device=None, dataset=None):
        ...
        optimizer_parameters = [
            {'params': [p for n, p in self.fmri_encoder.named_parameters() if not any(nd in n for nd in no_decay)], 'weight_decay': 1e-2, 'lr': lr_encoder},
            {'params': [p for n, p in self.fmri_encoder.named_parameters() if any(nd in n for nd in no_decay)], 'weight_decay': 0.0, 'lr': lr_encoder},
            {'params': [p for n, p in self.fa_module.named_parameters() if not any(nd in n for nd in no_decay)], 'weight_decay': 1e-2, 'lr': lr_encoder},
            {'params': [p for n, p in self.fa_module.named_parameters() if any(nd in n for nd in no_decay)], 'weight_decay': 0.0, 'lr': lr_encoder},
            {'params': [p for n, p in self.diffusion_prior.net.named_parameters() if not any(nd in n for nd in no_decay)], 'weight_decay': 1e-2, 'lr': lr_diffusion},
            {'params': [p for n, p in self.diffusion_prior.net.named_parameters() if any(nd in n for nd in no_decay)], 'weight_decay': 0.0, 'lr': lr_diffusion},
            {'params': [p for n, p in self.shape_generator.named_parameters() if "adapter" in n], 'weight_decay': 1e-2},
        ]
        for name, param in self.CLIP_Model.named_parameters():
            param.requires_grad = False
        for name, param in self.shape_generator.named_parameters():
    ...
    def get_loss(self, meta):
        ...
        fmri_embed = self.fa_module(x,b,f)
        ...
        diff_loss, fmri_embedding = self.diffusion_prior(fmri_embed = fmri_embed, image_embed = image_features)
        nll_loss = 0
        plane_index = meta['quantize_index'].cuda()
        logits_value = self.shape_generator.transformer_model(plane_index, fmri_embedding)
    ...
    def get_test_metric(self, meta):
        ...
        fmri_embedding = self.diffusion_prior.sample(fmri_embed, num_samples_per_batch = 4, cond_scale = 1.)
    ...
    def get_fmri_embedding_batch(self, fmri_data, diffusion_bs=16, decoder_bs=4, ddim_steps=50):
        ...
        x = self.fmri_encoder.forward_encoder_w_pred(x)
        fmri_embed = self.fa_module(x,b,f)
        fmri_embedding = self.diffusion_prior.sample(fmri_embed, num_samples_per_batch = diffusion_bs, cond_scale = 1.)
