...
from src.decoder.encoder.unet import UNet
from src.decoder.conv_onet.models.quantize_2D_improve_revise import VectorQuantizer


class Quantize_multi_improve_revise(nn.Module):
    '''
    ...
    def __init__(self, cfg, n_e, e_dim, c_dim, beta, unet_kwargs):
        super(Quantize_multi_improve_revise, self).__init__()
        self.n_e = n_e
        self.e_dim = e_dim      # Codebook Embedding dim
        self.beta = beta

        try:
            self.project_dim = cfg['3d_model']['quantizer_kwargs']['reduce_dim']
        except:
            self.project_dim = 8
