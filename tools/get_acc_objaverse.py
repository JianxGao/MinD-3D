...
def calculate_lpips(preds, gts, batch_size=64):
    """
    计算 LPIPS，相同 shape 的图像对按 batch 分批计算以避免 OOM。
    
    Args:
        preds (List[PIL.Image or np.ndarray]): 预测图像列表
        gts (List[PIL.Image or np.ndarray]): GT 图像列表
        batch_size (int): 每个 batch 的大小

    Returns:
        float: 所有图像对的平均 LPIPS 分数
    """
...
def main():
    ...
    pred_imgs, gt_imgs = load_images_from_dir(args.pred_root,"~/dataset/fmri_objaverse/images")
