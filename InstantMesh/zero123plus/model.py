...
from src.models.fmri_encoder.autoencoder import fMRI_Encoder
from src.models.fmri_encoder.layers import torch_init_model
...
class MVDiffusion(pl.LightningModule):
    def __init__(
        ...
        fmri_encoder_ckpt = torch.load("/ssd/gaojianxiong/checkpoint_120000_encoder.pth", map_location='cpu')["model"]
        ...
        state_dict = torch.load("/ssd/gaojianxiong/diffusion_pytorch_model.bin", map_location='cpu')
        new_ckpt = {"base_model.model.unet."+key: value for key, value in state_dict.items()}
        self.unet.load_state_dict(new_ckpt, strict=False)
    ...
    def prepare_batch_data(self, batch):
        ...
        # prepare fmri for stable diffusion input
    ...
    def encode_embed_fmri_condition_fmri(self, fmri):
        dtype = next(self.fmri_encoder.parameters()).dtype
        B = fmri.shape[0]
        latents = self.fmri_encoder.forward_encoder_wo_pred(fmri.to(dtype).view(-1, 1, 256, 256)) # [B*6, 257, 1024]
        ...
        ramp = global_embeds.new_tensor(self.pipeline.config.ramping_coefficients).unsqueeze(-1)
        encoder_hidden_states = encoder_hidden_states + global_embeds * ramp
        return global_embeds, latents # should be [B, 4, 64, 64]
    ...
    def training_step(self, batch, batch_idx):
        ...
        if np.random.rand() < self.drop_cond_prob:
            ...
            _, fmri_cond_latents = self.encode_embed_fmri_condition_fmri(torch.zeros_like(cond_fmri))
        else:
            # prompt_embeds = self.forward_vision_encoder(cond_imgs)
            # cond_latents = self.encode_condition_image(cond_imgs)
            prompt_embeds, fmri_cond_latents = self.encode_embed_fmri_condition_fmri(cond_fmri) # [2, 6, 64, 64]
    ...
    def validation_step(self, batch, batch_idx):
        ...
        #     save_image(grid, os.path.join(self.logdir, 'images_val', f'val_{self.global_step:07d}_{batch_idx}.png'))
