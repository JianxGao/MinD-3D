...
class Zero123PlusPipeline(diffusers.StableDiffusionPipeline):
    ...
    def __call__(
        ...
        **kwargs
        ...
        # latents = self.fmri_encoder.forward_encoder_wo_pred(fmri.to(self.vae.dtype).view(-1, 1, 256, 256)) # [6, 257, 1024]
        # embed_fmri, latents = latents[:,:1,:], latents[:,1:,:]
        # latents = latents.view(6, 16, 16, 4, 4, 64).permute(0,1,3,2,4,5)
        # latents = latents.reshape(6, 16 * 4, 16 * 4, 64)
        # latents = latents.permute(1, 2, 0, 3).reshape(1, 64, 64, 64 * 6)
        # cond_lat = self.fmri_encoder.aggregation(latents).permute(0, 3, 1, 2)
        # embed_fmri = embed_fmri.view(1, 6, 1, 1024)
